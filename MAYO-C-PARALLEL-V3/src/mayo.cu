              ...
              size_t mlen, const unsigned char *csk)
{
    ...
    cudaMallocHost((void**)&h_seed_pk,
                MAYO_2_pk_seed_bytes * BATCH);
    ...
    sk_t *d_sk;
    cudaMalloc((void **)&d_sk, BATCH * sizeof(sk_t));
    ...
    /* Unpack P1/P2 */
    unpack_m_vecs<<<blocks_unpack, THREADS>>>(
        d_P_bytes,
        d_P_limbs,
        total_vecs
    );
                               ...
                               const unsigned char *d_seed_sk)
{
    ...
    if (local < MAYO_2_digest_bytes) {
        ...
        /*
           Si cada batch tiene su propia csk:
           value = d_seed_sk[batch_id * MAYO_2_sk_seed_bytes + sk_idx];

           Si todos usan la misma csk, también funciona si replicaste d_seed_sk por batch.
        */
        value = d_seed_sk[batch_id * MAYO_2_sk_seed_bytes + sk_idx];
    }
    else {
