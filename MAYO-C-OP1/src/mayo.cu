                            ...
                            unsigned char *sol_found)
{
    ...
    /* The back-substitution below is left on a single thread: it is
       a small fraction of this kernel's cost compared to the
       elimination step above (already batched 8 rows at a time via
       mul_fx8_device), so splitting it further across threads is not
       worth the extra synchronization. */
    if (tid != 0) {
        return;
    }
