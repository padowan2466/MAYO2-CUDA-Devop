...
#define COMPUTE_A_THREADS (256)

__global__
void compute_A_fused_shared(
    const uint64_t *__restrict__ VtL,
    unsigned char *__restrict__ A_out
);
...
#define SAMPLE_PREPARE_THREADS 128

__global__
void sample_solution_prepare_shared(
...
uint64_t *d_L =
    ...
    compute_A_fused_shared
    <<<BATCH, COMPUTE_A_THREADS>>>(
        d_VL,
        d_A_out
    );

cudaError_t compute_A_error =
    cudaGetLastError();

if (compute_A_error != cudaSuccess)
{
    printf(
        "Error compute_A_fused_shared: %s\n",
        cudaGetErrorString(compute_A_error)
    );
}
        ...
        sample_solution_prepare_shared
    <<<BATCH, SAMPLE_PREPARE_THREADS>>>(
        d_r,
        d_x,
        d_A_out,
        d_y
    ...
    cudaError_t prepare_error =
        cudaGetLastError();

    if (prepare_error != cudaSuccess)
    {
        printf(
            "Error sample_solution_prepare_shared: %s\n",
            cudaGetErrorString(prepare_error)
        );
    }
    ...
    int total_tiles)
{
    ...
    int group = threadIdx.x / TILE_SIZE;  
...
{
    constexpr int KO =
        MAYO_2_k * MAYO_2_o;          // 68

    constexpr int M =
        MAYO_2_m;                     // 64

    constexpr int A_COLS =
        MAYO_2_k * MAYO_2_o + 1;      // 69
    ...
    __shared__ unsigned char s_r[KO];
...
void set_ctr_in_tmp_kernel(unsigned char *d_tmp, int ctr_group)
{
    ...
    int current_ctr = ctr_group * BATCH + b;
    if (current_ctr > 255) current_ctr = 255;

    d_tmp[b * tmp_bytes + tmp_bytes - 1] = (unsigned char)current_ctr;
}
...
void compute_A_fused_shared(
    ...
    unsigned char *__restrict__ A_out)
{
    constexpr int PARAM_K =
        MAYO_2_k;

    constexpr int PARAM_O =
        MAYO_2_o;

    constexpr int PARAM_M =
        MAYO_2_m;

    constexpr int M_VEC_LIMBS =
        MAYO_2_m_vec_limbs;
    ...
    constexpr int USEFUL_COLS =
        A_COLS - 1;

    
    constexpr int A_WIDTH =
        ((PARAM_K * PARAM_O + 15) / 16) * 16;

    
    constexpr int A_PACKED_ROW_BLOCKS =
        (PARAM_M + PAIRS + 15) / 16;

    
    constexpr int A_WORK_LIMBS =
        A_WIDTH * A_PACKED_ROW_BLOCKS;
    ...
    __shared__ uint64_t s_build[A_WORK_LIMBS];
    __shared__ uint64_t s_transposed[A_WORK_LIMBS];
    ...
    constexpr int BUILD_OPERATIONS =
        PAIRS *
        PARAM_O *
        M_VEC_LIMBS *
        2;

    for (int operation = tx;
         operation < BUILD_OPERATIONS;
         operation += blockDim.x)
    {
        int local = operation;
    ...
    for (int output_index = tx;
         output_index < A_WORK_LIMBS;
    ...
    constexpr int COLUMN_BLOCKS =
        A_WIDTH / 16;

    constexpr int REDUCE_OPERATIONS =
        COLUMN_BLOCKS * PAIRS;

    for (int operation = tx;
         operation < REDUCE_OPERATIONS;
         operation += blockDim.x)
    {
        int pair_row =
            operation % PAIRS;
        ...
        constexpr uint64_t LOW_BIT_IN_NIBBLE =
    ...
    constexpr int DECODE_ELEMENTS =
        PARAM_M *
        USEFUL_COLS;

    for (int index = tx;
         index < DECODE_ELEMENTS;
         index += blockDim.x)
    {
        int col =
            index % USEFUL_COLS;
