              ...
              size_t mlen, const unsigned char *csk)
{
    ...
    for(int rep = 0;rep <= REPEATS; rep++)
    {
        ...
        for (int ctr = 0; ctr <= 255; ctr++)
        {
            ...
            /*
             * d_A_work is accumulated into via atomicXor in
             * compute_A_build. Unlike d_VL/d_Pv/d_A above, it was only
             * ever cleared once before the ctr loop, so a counter that
             * fails sample_solution_finish left its partial XOR state
             * in place for the next counter to accumulate on top of --
             * XOR is not idempotent, so this silently corrupts any
             * counter after the first whenever ctr=0 doesn't succeed.
             * Clear it every iteration, same as the other scratch
             * buffers.
             */
            cudaMemset(d_A_work,
                    0,
...
/*
 * Block-cooperative constant-time echelon form.
 *
 * The original EF_device ran on a single thread per signature (one
 * block of 1 thread), keeping a nrows*row_len uint64 working copy of
 * A in thread-local memory (which the compiler cannot fit in
 * registers, so it silently spills to slow local/global memory) and
 * looping over all `nrows` rows sequentially for both the pivot
 * search and the row elimination.
 *
 * The row-elimination step (the dominant cost: up to MAYO_2_m rows
 * touched per pivot column) is fully data-parallel across rows, so
 * this version runs the kernel with MAYO_2_m threads per signature
 * (one thread per matrix row), keeps the working copy in on-chip
 * __shared__ memory shared by that whole warp group, and lets every
 * thread eliminate its own row in parallel. The pivot search keeps
 * the exact same masked/branchless selection logic as before (still
 * done redundantly by a single thread, since it is an inherently
 * sequential scan and is cheap relative to the elimination step) so
 * the constant-time behaviour and bit-for-bit result are unchanged.
 */
__device__
void EF_device_cooperative(unsigned char *A, int tid)
{
                            ...
                            unsigned char *sol_found)
{
    ...
    /*
     * Back-substitution used to run entirely on tid 0 while the other
     * 63 threads in the block sat idle: profiling showed this single
     * thread costs roughly as much as the whole parallel elimination
     * step above (~270us out of ~540us on an RTX 3070). The row loop
     * itself must stay sequential (row R's column search reads
     * A_b[R][last], which lower rows' eliminations may have already
     * written), but for a fixed row the small window of candidate
     * columns (still found redundantly by tid 0, same as before, to
     * keep that scan branchless/constant-time) all target the exact
     * same set of disjoint 8-row groups below `row`. Those groups are
     * independent of each other, so each is handed to one thread,
     * which folds every column's contribution into its own 8 bytes
     * with plain (non-atomic) XORs since no other thread ever touches
     * that group.
     */
